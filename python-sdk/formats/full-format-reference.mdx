...
If you'd like to upload annotations along with your data using either the [CLI](/python-sdk/cli-overview/import-data-and-annotations) or the [SDK](/python-sdk/sdk-overview/importing-data-and-annotations), please see the corresponding documentation.
...
The items entry is a list of file paths that point to your data. Please have a look at the [Items List](/importing-data/import-cloud-data#items-list)documentation for a fuller explanation of how to format for various modalities and series/study uploads.

#### `name: string`

An optional user-defined string, *needs to be unique* across all series. Individual [series will be named after this value on the labeling tool](https://www.loom.com/i/ea12e486bd8845d7b3b8a83fc115ad58). Exported segmentation files will also be named using this value. Using the Series Instance UID here is good practice.
...
#### `pointTopLeft: `[`Point2D`](/python-sdk/formats/full-format-reference#point2d-xnorm-number-ynorm-number)
...
#### `points: `[`Point2D`](/python-sdk/formats/full-format-reference#point2d-xnorm-number-ynorm-number)`[]`

A list of 2D points that are connected to form a polygon. This list is ordered such that, pointi​ is connected to pointi+1​. The last point is also connected to the first point to close the polygon.
...
#### `point1, point2 : `[`VoxelPoint`](/python-sdk/formats/full-format-reference#voxelpoint-i-number-j-number-k-number)

A length measurement is defined by two points, and the length measurement is the distance between the two points.

#### `absolutePoint1, absolutePoint2 : `[`WorldPoint`](/python-sdk/formats/full-format-reference#worldpoint-x-number-y-number-j-number)
...
#### `point1, point2, vertex : `[`VoxelPoint`](/python-sdk/formats/full-format-reference#voxelpoint-i-number-j-number-k-number)
...
The [series entry](/python-sdk/formats/full-format-reference#series) for the current user only.
...
[PreviousFormats](/python-sdk/formats)[NextExport Structure](/python-sdk/formats/export-structure)
