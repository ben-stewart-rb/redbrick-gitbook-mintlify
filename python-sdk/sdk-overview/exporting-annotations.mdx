...
description: "You can make use of RedBrick AI's Python SDK to export your annotations using a Python script."
---
...
The segmentation directory will contain a single sub-directory for each task in your export. The sub-directories will be named after the task [`name`](/python-sdk/sdk-overview/exporting-annotations#name-string). A single task (depending on whether it was single series or multi-series) can have one or more segmentations.

The individual segmentation files will be in NIfTI-1 format and be [named after the user-defined series name](/python-sdk/sdk-overview/exporting-annotations#name-string-1). If no series name is provided on upload, RedBrick will assign a unique name. Corresponding meta-data ex. category names will be provided in [tasks.json](/python-sdk/sdk-overview/exporting-annotations#tasks-json).

***

## Code Examples

As always, you should first perform the [standard RedBrick AI SDK setup](/python-sdk/sdk-overview#initializing-the-redbrick-sdk-in-python) to create a Project object.
...
First, perform the [standard RedBrick AI SDK setup](/python-sdk/sdk-overview#initializing-the-redbrick-sdk-in-python) to create a Project object.
...
* Upload a [script](https://redbrick-sdk.readthedocs.io/en/stable/sdk.html#redbrick.settings.Settings.label_validation) for [Custom Label Validation](/project-settings-and-configuration/custom-label-validation);

[PreviousAssigning & Querying Tasks](/python-sdk/sdk-overview/assigning-and-querying-tasks)[NextCLI Overview](/python-sdk/cli-overview)
